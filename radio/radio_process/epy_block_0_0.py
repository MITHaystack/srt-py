...
class clk(gr.sync_block):
    ...
    def work(self, input_items, output_items):
        ...
        while ((nitems - self.d_offset) > self.d_nsamps):
            self.d_offset += self.d_nsamps
            self.add_item_tag(0, self.d_offset, self.pmt_key, make_time_pair(time.time()))
